...
const personHouseholdLinksRoute = new Hono<{
  Bindings: Env;
  Variables: { user: import('../middleware/firebase-auth').AuthUser };
}>();

personHouseholdLinksRoute.use('/*', firebaseAuth());
...
personHouseholdLinksRoute.post('/', requireRole('admin', 'imam'), async (c) => {
  ...
  const person = await db
    .select()
    .from(persons)
    .where(and(eq(persons.id, body.person_id), eq(persons.tenant_id, user.tenant_id!)));
  const household = await db
    .select()
    .from(households)
    .where(and(eq(households.id, body.household_id), eq(households.tenant_id, user.tenant_id!)));
...
// DELETE /api/person-household-links/:id — Unlink a person from a household (soft delete/history)
personHouseholdLinksRoute.delete('/:id', requireRole('admin', 'imam'), async (c) => {
  const db = createDb(c.env.DATABASE_URL);
  const id = c.req.param('id') as string;
  const body = await c.req.json().catch(() => ({})); // optional body for end_date

  // Set end_date to provided date or today
